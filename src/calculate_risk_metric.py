...
def calculate_risk_metric(ticker, start_date):
    ...
    price_per_risk = {
        round(risk, 1): round(np.exp(
            (risk * (df['Preavg'].cummax().iloc[-1] - (cummin := df['Preavg'].cummin().iloc[-1])) + cummin) / df.index[
                -1] ** diminishing_factor + np.log(df['MA'].iloc[-1])
        ))
...
if __name__ == "__main__":
    ...
    df_results['Risk to MA'] = ((df_results['MA'].astype(float) - df_results['value'].astype(float)) / df_results[
    ...
    styled_df = df_results.style.applymap(lambda x: 'color: green' if x < 0.5 else (
        'color: red' if x > 0.8 else ('color: orange' if 0.6 < x < 0.8 else ('color: blue' if x < 0.2 else ''))),
                                          subset=['avg'])
    print(styled_df)
